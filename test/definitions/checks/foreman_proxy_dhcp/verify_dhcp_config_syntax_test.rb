...
describe Checks::ForemanProxyDhcp::VerifyDhcpConfigSyntax do
  include DefinitionsTestHelper

  subject do
    Checks::ForemanProxyDhcp::VerifyDhcpConfigSyntax.new
  end

  it 'passes when no any error in syntax as well as DHCP subnets' do
    assume_feature_present(:foreman_proxy_dhcp, :valid_dhcp_configs? => true)
  ...
  it 'fails when failure either in syntax or in DHCP subnets' do
    assume_feature_present(:foreman_proxy_dhcp, :valid_dhcp_configs? => false)
    result = run_check(subject)
