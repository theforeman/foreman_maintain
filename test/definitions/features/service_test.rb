...
describe Features::Service do
  ...
  def service_names_match?(filtered_services, services_to_compare)
    _(filtered_services.values.flatten(1).map(&:name)).must_equal services_to_compare.map(&:name)
  end
  ...
  describe 'existing_services' do
    it 'returns list of sorted existing services (local)' do
      ...
      _(subject.existing_services).must_equal [local_candlepin_db, httpd]
    end
    ...
    it 'returns list of sorted existing services (remote)' do
      ...
      _(subject.existing_services).must_equal [remote_candlepin_db, remote_foreman_db, httpd]
    end
  end
  ...
  describe 'filtered_services' do
    ...
    it 'returns the same set without any filters' do
      ...
      _(services_array).must_equal [remote_candlepin_db, remote_foreman_db, httpd]
    end
    ...
    it 'the :only filters all services regardless on component' do
      ...
      _(subject.filtered_services(:only => ['postgresql'])).must_equal remote_dbs
    end

    it 'the :only accepts also list of SystemServices and filter by component' do
      remote_cpdb = { 10 => [remote_candlepin_db] }
      _(subject.filtered_services(:only => [remote_candlepin_db])).must_equal remote_cpdb
    end

    it 'applies the exclude filters' do
      httpd_service = { 30 => [httpd] }
      _(subject.filtered_services(:exclude => ['postgresql'])).must_equal httpd_service
    end
