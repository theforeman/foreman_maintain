module ForemanMaintain
  module Cli
    class TaskCleanupCommand < Base
      ...
      option %w[-B --batch-size], 'BATCH_SIZE',
             'Process tasks in batches of BATCH_SIZE, 1000 by default' do |s|
        Integer(s)
      end
      option %w[-S --states], 'STATES',
             'Operate on tasks in STATES, comma separated list of states, ' \
             'set to all to operate on tasks in any state' do |s|
        s.split(',')
      end
      option %w[-a --after], 'AGE',
             'Operate on tasks older than AGE. Expected format is a number ' \
      ...
      option %w[-r --rake-command], 'RAKE_COMMAND',
             'Use RAKE_COMMAND as rake', :default => DEFAULT_RAKE_COMMAND
